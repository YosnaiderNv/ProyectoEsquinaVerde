...
if (isset($_POST['enviar'])) {
    ...
    if (!empty($nombre) && !empty($valor) && !empty($codigo) &&  !empty($fecha)&& !empty($descripcion) && !empty($ruta) ) {
        ...
        $query = "INSERT INTO tb_productos(nombre, valor, codigo, fecha, descripcion, ruta VALUES ('$nombre ','$valor','$codigo','$fecha','$descripcion ','$ruta')";

        $resultado =$conexion->query($query);

        if ($resultado) {
           
            header("refresh:4;url=../Vistas/crearProductos.php");
            mostrarHtml1();
            exit();
